...
| 10 | **本地验证结论回写 `hk_vps_4/specs/` 技术方案** | 任务 | 文档 | 本 Sprint 的实测结论（`[limits]` 对 stdio 是否生效、`gc` 覆盖范围、写路径泄露结论、隔离验证结果、档位实际暴露数）全部回写到对应 spec；各文件追加变更记录 | `hk_vps_4/specs/` 全目录 | ☐未开始 |
...
| 2 | OSS 私有桶（香港 region + SSE）+ RAM 子账号 AK | 阻塞 | 安全 | 桶为私有 + SSE 已开；AK 仅存服务器侧，不入仓 | [`mcp_oss_bak_com_requirements.md`](./mcp_oss_bak_com_requirements.md) | ⛔待提供 |
| 3 | **编写 `hk_vps_4/backup/` 备份脚本** | 任务 | 部署 | `backup-and-push.sh`（快照 → sha256 → ossutil 上传 → **回读比对** → 失败非零退出）与 `restore-drill.sh`（拉最新 → 校验 → restore → `doctor` 通过）可重复执行；`make backup` / `make restore-drill` 可用；**遍历 `/data/users/*`** | `product-backlog.md` #9 · `dev-plan.md` §4.3 | ☐未开始 |
...
| 2026-09-20 | **新增 [`mcp-test.md`](./mcp-test.md)（MCP 测试策略+计划+用例的 spec，用户指示设立）**：① 传输形态核实（源码级）：生产调用方式为 **stdio-over-SSH**，**不是 http url + API key**——上游正式路由无 `/mcp` `/sse`（仅 `/api/v1/*`），`server.json` 只声明 stdio；若未来开 HTTP 入口须按 `deployment_strategy.md` §8 补 api_key；② 固化现役用例（mcp-smoke 6 条 + doctor 2 条，含退出码语义）；③ 登记本地/生产 Cursor 接入配置（`ai-memory-local` = `docker exec -i`；`-i` 不加 `-t`；放全局 mcp.json 不放项目级）；④ 占位 Sprint 3 档位 / Sprint 5 SSH 通道 / HTTP 开放后的远程用例 |
