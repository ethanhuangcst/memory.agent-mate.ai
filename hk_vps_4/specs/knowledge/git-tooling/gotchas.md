...
tags:
  ...
  - git-grep
  - secret-scanning
related_spec: hk_vps_4/specs/asset_isolation_plan.md
related:
...
- 正确写法：注释单独成行。
  ...
  secrets.local*
  ```
- 验证：`git check-ignore -v hk_vps_4/secrets.local.hk_vps_4.md` 应输出 `.gitignore:N:secrets.local*`。
...
## Links
- 护栏实现：`hk_vps_4/scripts/secret-check.sh` · 钩子：`hk_vps_4/scripts/git-hooks/pre-commit`
- 决策：ADR-006
