...
Sprint 2 #5 的验收要求给出「多用户隔离是否可实现」的明确结论（`specs/multiuser_isolation.md` §0）。结论是**有条件可实现**，但结论成立需要一个前提：隔离的**形态**必须先定死，且不能是"看起来可以、写路径不可信"的那种。
...
6. **探针常设**：`hk_vps_4/scripts/iso-probe.sh` 作为该决策的常设回归证据（A 负向解析链 / B 方案③双用户隔离 / C 方案②对照；语义化退出码；可重复运行，含 near-duplicate CONFLICT 幂等分支）。本决策的每一条都可以被它重跑证伪。
...
- **成本**：每用户库需要各自的周期维护（`--db <path> gc` / `curator --once`）与**逐一备份**（外迁前缀 `tenants/<handle>/`）；`gc` 是否覆盖每库的 TTL 遗忘与 WAL checkpoint 仍属未核实项（Sprint 3 #7）。
- **对门户的约束**：必须"一会话一子进程、禁止跨用户复用/池化"（D3），且 key 的创建/轮换/吊销要同步管理库目录与 `authorized_keys` 行。
- **仍未关闭的上游风险**：`AI_MEMORY_DB` 若被错设为一个**有效但错误**的他库路径（非空、非 `/data/users/`），sqlite 不会报错 —— 只能靠 D1 的路径断言拦住（已登记为 Sprint 3 #5 的输入）。
- **待定**：`<handle>` 命名规范（大小写/长度/是否等于邮箱别名）与 sudoers 无通配符 argv 匹配的实测（`specs/multiuser_isolation.md` §0.4）。
- **本 ADR 不实施任何防线**：D1–D5 的落地在 Sprint 3 #5 / #6，本 ADR 只固定形态与判据。
