...
3. **维度一律实测**：新增 `hk_vps_4/scripts/qwen-verify.sh`，探测端点 `/models` → 真实 chat → `json_object` → embeddings，并以响应向量长度作为 `dim`。
4. **本地部署**新建 gitignored 的 `hk_vps_4/deploy/.env.local` + `config.local.toml`；生产侧 `config.toml.tmpl` 保持同结构、端点用占位符 `<QWEN_BASE_URL>`。
