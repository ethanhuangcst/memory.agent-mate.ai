...
- **镜像**：`ghcr.io/alphaonedev/ai-memory:0.9.0`（固定版本，禁用 `latest`；消费上游官方镜像 = 已批准例外，本仓库无构建工作流）
...
| 5 | 部署仓库建立并 push（见 asset_isolation_plan.md §6） | ☐ 待执行 |
...
### 5.1 七步链路（人工触发；自动仅到「提醒」）

```text
① 版本跟踪 → ② 预检 → ③ 本地灰度 → ④ 生产升级 → ⑤ 验证 → ⑥ 回滚预案 → ⑦ 文档同步
```

| 步 | 动作 |
| --- | --- |
| ① | 跟踪上游 release（半自动：部署仓库 scheduled Action 每日比对上游 latest release 与 `IMAGE_TAG`，不一致自动开 issue 附 CHANGELOG 链接。**部署决策与执行始终人工**） |
| ② | **预检**：读 CHANGELOG，按 §5.2 三类清单逐项核对 |
| ③ | **本地灰度**：拉新镜像，用**生产快照副本**先跑迁移（scratch migration 工作流），观察迁移日志 |
| ④ | 改 `IMAGE_TAG` → Portainer Recreate（勾选重新拉取）→ 迁移自动执行（自动生成 pre-migration 快照） |
| ⑤ | 验证：doctor 三静默失败点 + §3.3 冒烟 + **§5.3 备份管线探针** |
| ⑥ | 回滚预案：pre-migration 快照覆盖 `/data/ai-memory.db`（清 `-wal`/`-shm`）+ `IMAGE_TAG` 改回旧版（上游拒绝启动于更新的库 → 回滚失败大声报错） |
| ⑦ | 同步 `deployment_strategy.md` 版本记录 + `deployment-plan.md` 的 IMAGE_TAG ↔ 上游 tag 映射 |

### 5.2 升级门禁与三类破坏性变更

**门禁：无新鲜外迁备份，不升级。** 升级前确认 OSS 上最新快照时间戳与 sha256（一次 `ossutil ls` + 比对）。

| 类别 | 历史证据 | 核对动作 |
| --- | --- | --- |
| **secure-by-default 翻转** | v0.9 把 attestation 翻成默认开启（我们设 `=0`） | 重验 `.env` / `config.toml` 的每项假设仍成立 |
| **config schema 变更** | legacy flat 字段已移除；有 `ai-memory config migrate` | 跑 migrate --dry-run 看差异 |
| **CLI / MCP 契约变更** | `backup`/`restore` 参数、profile 工具数（core=7）、Cursor 40 工具上限 | 核对 Cursor MCP args、curator/CLI 调用 |

### 5.3 升级后备份管线探针（必做）

手动跑一次 `ai-memory backup` + 确认 ossutil 上传成功 —— 这一步验证备份管线在新二进制下仍工作，是 `backup` CLI 契约变更的检测探针。

### 5.4 与备份外迁脚本的耦合点

`backup-and-push.sh` / `restore-drill.sh` 直接调用 `ai-memory backup` / `restore` —— 上游改这两个命令的参数/行为即断裂。**列入每次升级预检**；脚本内置 `--help` 快照比对可提前发现。
...
| 5 | 上游 secure-default 再翻转 | §5.2 预检清单第一优先级 |
| 6 | 公开仓库泄露 secrets | asset_isolation_plan.md §8 脱敏规则 + gitleaks |
...
| # | 动作 | 依赖 |
| --- | --- | --- |
| 1 | 父仓初始化 + 资产迁移（asset_isolation_plan.md §8；含 `.gitignore` 护栏与 Makefile） | ☐ |
