...
- **端点**：走**私有 MaaS workspace** —— `[llm].base_url` 与 `[embeddings].base_url` 必须显式覆盖（公开仓用占位符 `<QWEN_BASE_URL>`，真实值见 `memory.agent-mate.ai/secrets.local.hk_vps_4.md`）。`qwen` 别名默认指向公网 dashscope，而 workspace 级 key 在公网端点不通
