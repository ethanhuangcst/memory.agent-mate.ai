...
| <a id="pb-14"></a>14 | [产品化-MCP] | mcp | 定制 — 接入面 | HTTP MCP（门户，**新增**）+ SSH stdio（主人自用，门户故障时的**保底**）。保留 SSH 路径的目的是**降级不失效**：门户挂掉时主人仍能读写自己的记忆。 | 两条路径都能完成 MCP 握手并成功读写；门户严格一会话一子进程，禁止跨用户复用或池化；停掉门户后 SSH 路径仍可用。 | [`web-portal/web-stories.md`](./web-portal/web-stories.md) S3 · [`web-portal/web-design.md`](./web-portal/web-design.md) §2 · [`mcp/mcp-design.md`](./mcp/mcp-design.md) §5.1 · [Sprint 6「接入面」](./sprint-plan.md#s5-access-surfaces) · [Sprint 4 PSP-W2「端到端接入」](./sprint-plan.md#s4-mcp-session-bridge) | Sprint 6 | ToDo |
