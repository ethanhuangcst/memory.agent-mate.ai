...
| 13 | [产品化-MCP] | mcp | 定制 — 每用户库的后台维护 | 每用户库的 TTL 遗忘 / 压缩 / GC / curator **由谁跑** —— 既有常驻 `curator` **只服务默认库**。需定：门户调度 or 主机 cron；并核实 `gc` 是否覆盖每库的 TTL 遗忘与 WAL checkpoint（该点在方案文档中标为**待核实**，不可照抄假定）。 | 调度落地并留日志；逐库 `ai-memory --db <each> gc` 与 `curator --once` 执行成功；TTL 遗忘与 checkpoint 覆盖**已实测核实**。 | [`mcp/mcp-design.md`](./mcp/mcp-design.md) §5.3 | Sprint 3 | ToDo |
