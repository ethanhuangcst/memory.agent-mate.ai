...
| 4 | [产品化-web] | web portal | admin portal — 记忆身份与数据隔离 | 每用户 `AI_MEMORY_AGENT_ID=human:<handle>` + 独立库 `/data/users/<handle>/ai-memory.db`（**一用户一库**，读写双向物理隔离）。**不做**服务器 OS 账号同步（单账号 `aimem-ssh` + N 把密钥 + N 条 forced command）。建用户时创建其库目录（属主对齐容器内 `aimem`）；签发 key 时**幂等确保**目录存在；**库文件由 ai-memory 首次使用时自动创建**。 | 跨用户检索**命中不到**；`memory_get` 他用户记忆返回不可见；每个库文件属主为 `aimem`。 | [`mcp/mcp-design.md`](./mcp/mcp-design.md) §5 · [`web-portal/web-stories.md`](./web-portal/web-stories.md) S4 | Sprint 4 | ToDo |
| 5 | [产品化-web] | web portal | admin portal — 审计视图 | 覆盖事件：建用户 / 签发 / 轮换 / 吊销 / 每次会话开始（**含解析出的库路径**，便于事后对账「这次会话落在哪个库」）。 | 上述事件均可查询；日志中**不出现**令牌明文。 | [`web-portal/web-stories.md`](./web-portal/web-stories.md) S5 · [`web-portal/web-test.md`](./web-portal/web-test.md) §2 | Sprint 4 | ToDo |
...
| 14 | [产品化-MCP] | mcp | 定制 — 接入面 | HTTP MCP（门户，**新增**）+ SSH stdio（主人自用，门户故障时的**保底**）。保留 SSH 路径的目的是**降级不失效**：门户挂掉时主人仍能读写自己的记忆。 | 两条路径都能完成 MCP 握手并成功读写；停掉门户后 SSH 路径仍可用。 | [`web-portal/web-stories.md`](./web-portal/web-stories.md) S3 · [`web-portal/web-design.md`](./web-portal/web-design.md) §2 · [`mcp/mcp-design.md`](./mcp/mcp-design.md) §5.1 | Sprint 5 | ToDo |
| 15 | [产品化-MCP] | mcp | 定制 — 可用性前提（agent attestation） | `AI_MEMORY_REQUIRE_AGENT_ATTESTATION=0` —— 不设则无签名写入 **403**（v0.9 的 require-everywhere 缺陷；v0.10 已改为 surface-scoped）。**所有** spawn 路径（门户模板、SSH forced command、compose、cron）都必须带上。 | 各路径均设置该 env；漏设时能**显式报错**而非静默失败。 | `deploy/docker-compose.prod.yml` · [`mcp/mcp-design.md`](./mcp/mcp-design.md) §9 | Sprint 3 | WIP |
...
| 26 | [上线] | 上线生产环境 | 部署 · 验证 · 文档化 | **部署方案** = `deployment.md` §3（ai-memory）+ [`web-portal/web-design.md`](./web-portal/web-design.md) §11（门户，含新增公网入口、反代与域名）；**验证方案** = `deployment.md` §7.3 冒烟 + [`mcp/mcp-design.md`](./mcp/mcp-design.md) §7 + [`web-portal/web-test.md`](./web-portal/web-test.md) §3；**文档化** = 本仓 `memory.agent-mate.ai/specs/`。 | 部署后上述三份验收清单全部通过。 | `deployment.md` §3 · [`web-portal/web-design.md`](./web-portal/web-design.md) §11 · [`web-portal/web-test.md`](./web-portal/web-test.md) §3 | Sprint 5 | ToDo |
