...
| <a id="pb-4"></a>4 | [产品化-web] | web portal | admin portal — 记忆身份与数据隔离 | 每用户 `AI_MEMORY_AGENT_ID=human:<handle>` + 独立库 `/data/users/<handle>/ai-memory.db`（**一用户一库**，读写双向物理隔离）。**不做**服务器 OS 账号同步（单账号 `aimem-ssh` + N 把密钥 + N 条 forced command）。建用户时创建其库目录（属主对齐容器内 `aimem`）；签发 key 时**幂等确保**目录存在；**库文件由 ai-memory 首次使用时自动创建**。 | 门户 spawn 前断言 `AI_MEMORY_DB` 非空、位于 `/data/users/` 且与当前 handle 绑定，不满足即拒绝会话；跨用户检索**命中不到**；`memory_get` 他用户记忆返回不可见；每个库文件属主为 `aimem`。 | [Sprint 4「门户 ↔ MCP 会话桥」](./sprint-plan.md#s4-mcp-session-bridge) · [`mcp/mcp-design.md`](./mcp/mcp-design.md) §5 · [`web-portal/web-stories.md`](./web-portal/web-stories.md) S4 | Sprint 4 | ToDo |
| <a id="pb-5"></a>5 | [产品化-web] | web portal | admin portal — 审计视图 | 覆盖事件：建用户 / 签发 / 轮换 / 吊销 / 每次会话开始（**含解析出的库路径**，便于事后对账「这次会话落在哪个库」）。 | 上述事件均可查询；每次会话开始事件包含最终解析的用户库路径；日志中**不出现**令牌明文。 | [Sprint 4「审计视图」](./sprint-plan.md#s4-audit-view) · [`web-portal/web-stories.md`](./web-portal/web-stories.md) S5 · [`web-portal/web-test.md`](./web-portal/web-test.md) §2 | Sprint 4 | ToDo |
...
| 18 | [产品化-MCP] | mcp | 定制 — 升级治理 | 版本契约（[`../upstream.lock`](../upstream.lock)）+ 准入判据 + **门户镜像随上游重建**：门户镜像 `COPY` 上游官方镜像里的二进制，故上游升级必须**重建门户镜像**，否则门户与部署制品版本漂移，而漂移是静默的。 | 升级清单含「重建门户镜像」一步，且已被演练过。 | [`web-portal/web-design.md`](./web-portal/web-design.md) §11 · [`web-portal/web-stories.md`](./web-portal/web-stories.md) S11 / S12 | Sprint 4 | ToDo |
...
| 21 | [升级] | 上游更新与升级 | 上游产品资产更新 — 准入判据与邮件提醒 | 判据已分层落地（**H1–H5 硬性阻断** / **W1–W6 人工确认**）；自动化**仅做到**每日比对 + 开 GitHub issue，**邮件提醒尚未实现**。 | 收到一封真实邮件（含版本号、判据结论、CHANGELOG 摘要）。 | `deployment.md` §9 · `adr/ADR-005-upgrade-admission-gate-layering.md` | Sprint 6 | WIP |
...
| 24 | [升级] | 上游更新与升级 | 制定部署/升级方案 — 制定方案与脚本 | `deployment.md` §9 七步链路已定；**相关脚本待补**。 | 相关脚本落地且可重复执行。 | `deployment.md` §9 | Sprint 6 | WIP |
...
| <a id="pb-26"></a>26 | [上线] | 上线生产环境 | 部署 · 验证 · 文档化 | **部署方案** = `deployment.md` §3（ai-memory）+ [`web-portal/web-design.md`](./web-portal/web-design.md) §11（门户，含新增公网入口、反代与域名）；**验证方案** = `deployment.md` §7.3 冒烟 + [`mcp/mcp-design.md`](./mcp/mcp-design.md) §7 + [`web-portal/web-test.md`](./web-portal/web-test.md) §3；**文档化** = 本仓 `memory.agent-mate.ai/specs/`。 | 部署后上述三份验收清单全部通过；生产模板必须先通过 V1 负向验证，失败即阻断上线。 | [Sprint 3「隔离本地负向回归」](./sprint-plan.md#s3-isolation-negative-regression) · [Sprint 5「上线验收」](./sprint-plan.md#s5-production-acceptance) · `deployment.md` §3 · [`web-portal/web-design.md`](./web-portal/web-design.md) §11 · [`web-portal/web-test.md`](./web-portal/web-test.md) §3 | Sprint 5 | ToDo |
