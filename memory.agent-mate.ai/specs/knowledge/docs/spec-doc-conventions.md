...
10. **编号空间会撞名（2026-09-21 实测）**：`web-portal/web-stories.md` 重写后引入故事号 S1–S13，而仓内早已存在两套**同形**编号 —— `deployment.md` §7.2 与 `web-portal/web-design.md` §5 的「**静默失败点** S1–S4」、`../scripts/probes/i18n-probe.sh` 的测试项标签 `S1–S12`。用 `grep` 做引用清扫时三者会同时命中：既可能把「静默失败点 S4」当成故事 S4 误改，也可能因命中噪声而漏判真正的错指。本轮实测正是后者 —— `product-backlog.md` #7 / #16 / #18 与 `sprint-backlog.md` Sprint 4 #6 / #8 共 **5 个条目**错指（S6 被写成 S3、S10 被写成 S9、S12 被写成非法的 `S9 #7`），而 S13 零引用，全部被同形编号掩盖。
...
16. **「零残留」必须写明扫描面（同日实测）**：重排把 8 份文档的约 46 处 Sprint 引用改指，并声明「全仓现行行零残留」，但 `../scripts/probes/limits-probe.sh` 的输出串与 `../scripts/maintain-user-dbs.sh` 的注释仍在引用旧编号 —— **脚本与制品不在「文档」的默认扫描面内**；声明的覆盖面若不含它们，就是自证不实。
