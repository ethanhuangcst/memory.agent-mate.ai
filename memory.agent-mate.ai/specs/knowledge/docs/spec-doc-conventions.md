...
本仓自有 spec 的写作与维护约定，来自 2026-09-21 至 2026-09-22 的三轮实证：对外能力文档的体例重构、旧 spec 引用清理，以及**部署文档与部署制品的逐字段收敛**。核心四条 —— **对外说明按「增量」组织并给全档连续编号**、**凡涉及实测的描述必须与被引实测逐项对齐**、**删并文档后靠护栏把引用清零，而不是靠人工记忆**、**配置与契约类文档必须与制品逐键互为镜像（双向可断言）**。
