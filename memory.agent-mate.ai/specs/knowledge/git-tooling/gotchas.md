...
- 场景：`link-check.sh` 的允许清单按「相对产品目录」书写（`specs/sprint-backlog.md`），脚本却按「相对仓根」（`memory.agent-mate.ai/specs/sprint-backlog.md`）匹配 → 键永不命中。
...
### 5. 全仓「风格/文本」类批量改动：先界定边界，再处理「原文引用」

- **边界**：任何全仓文本批处理（`sed` / 脚本扫 `*.md`）**必须排除 `ai-memory-mcp/`** —— 它是 vendored 上游、自带 `.git`，改它会在上游升级时变成 rebase 冲突源。按「自有资产目录（`memory.agent-mate.ai/`）」划界，而不是「仓内所有 md」。
- **原文引用**：文档里以代码块引用的**程序输出 / 话术**（如 `adr/ADR-005` 的「提示话术」= `scripts/upstream-preflight.sh` 的 stdout），**不能只改文档** —— 否则文档与真实输出永久不符。要么同批改脚本，要么在文档里显式登记为「例外 + 理由」。离线自测（`scripts/tests/run-fixtures.sh`）只断言**退出码 + 关键字**，不含装饰符，故改脚本输出是安全的。
- **批量替换的次生瑕疵**：删标记后要复查三类残留 —— 相邻粗体（`。** **上游原生支持**`）、双冒号（`注意：视情况：…`）、语义被删空的单元格（`✅`/`❌` 单独成格须补 `是` / `否`，否则表格缺词）。**先导出 diff 逐行看，再收口排版**。
...
- 决策：ADR-006（脱敏）· ADR-010（specs 结构与链接纪律）· ADR-011（文档风格：状态语义由文字承载）
