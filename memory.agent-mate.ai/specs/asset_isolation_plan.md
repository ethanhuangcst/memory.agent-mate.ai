...
| 2026-09-20（现版） | 改采**协同布局**：工程目录更名为 `memory.agent-mate.ai`，上游 clone 嵌套为子目录，自有资产集中在 `memory.agent-mate.ai/`。理由：单一工作区，文档/脚本/代码都在手边；隔离目标（上游纯净、自有资产独立版本化）同样达成 |
...
**判据**：任何一个文件，要么属于上游（`ai-memory-mcp/` 内、被其跟踪、未修改），要么属于本项目（`memory.agent-mate.ai/` 内、被父仓跟踪）——**不存在第三种位置**。

**根目录例外（仅两类；内容仍属本项目，位置由基础设施/平台决定）**：

1. `.gitignore`、`Makefile` —— 仓级基础设施，必须在根。
2. `.github/workflows/`、`.github/scripts/` —— **GitHub 平台强制**工作流位于仓库根，无法下沉到 `memory.agent-mate.ai/`（见 `.github/workflows/upstream-track.yml`）。

> 决议记录与知识库（`adr/`、`knowledge/`）现位于 `memory.agent-mate.ai/specs/` 之下，**属自有资产、非根目录例外**（2026-09-20 由仓根 `specs/` 并入，见 §12）。
...
| 2 | pre-commit 守卫：扫描暂存区公网 IP（`secret-check.sh` via `make hooks-install`，已实现）；拒绝 stage `ai-memory-mcp/` 由护栏 1 gitignored 天然阻挡 | 已实现（`memory.agent-mate.ai/scripts/secret-check.sh` + `.git/hooks/pre-commit`；可用 `git commit --no-verify` 绕过，纪律靠评审） |
| 3 | 上游 remote 非本方所有 → **天然无推送权** | 无需额外机制 |
| 4 | 每次上游升级后，父仓 `git status` 必须仍然干净 | 写入 dev-plan.md §5 升级预检清单 |
| 5 | 父仓记录上游对应版本 | **`memory.agent-mate.ai/upstream.lock` 为唯一真相源**（上游 tag / commit / 镜像 digest / schema / 沉淀期阈值）；`deployment-plan.md` §3.1 提供人读映射表；`make pin` 打印、`make pin-update` 回写 |
...
- 「想改上游文件」的冲动出现时，那个改动应当落在 `memory.agent-mate.ai/` 的覆盖层（配置/补丁/文档结论），而不是改 clone——此为**团队约定**，写入部署仓库 README。

## 5. 隔离规则（长期生效）

1. **clone 内禁止修改任何被跟踪文件**；需要「覆盖」上游行为的一律放 `memory.agent-mate.ai/`，并注明对应上游版本。
...
3. **父仓记录上游对应版本**：**唯一真相源为 `memory.agent-mate.ai/upstream.lock`**（tag / commit / 镜像 digest / schema）；`make pin` 打印、`make pin-update` 回写。
...
backup:              ## bash memory.agent-mate.ai/backup/backup-and-push.sh
restore-drill:       ## bash memory.agent-mate.ai/backup/restore-drill.sh
...
4. 自有资产移入 `memory.agent-mate.ai/`：`docs/ye_cao_yun_production/*` → `memory.agent-mate.ai/specs/`；`docs/ye_cao_yun_production/deploy/*` → `memory.agent-mate.ai/deploy/`
5. 写 `.gitignore`（`ai-memory-mcp/`）→ **先于任何 `git add`**
6. 修正文档内引用：仓内互引改为 `memory.agent-mate.ai/...` 相对路径；上游引用改为 GitHub URL + 固定 tag
7. 验证：父仓 `git status` 仅出现 `memory.agent-mate.ai/` 与 Makefile；`ai-memory-mcp/` 内 `git status` 干净、`git pull` 可用
...
| 2 | `deployment_strategy.md` | → `memory.agent-mate.ai/specs/` | ✅ |
| 3 | `dev-plan.md` | → `memory.agent-mate.ai/specs/` | ✅ |
| 4 | 本文档 | → `memory.agent-mate.ai/specs/` | ✅ |
| 5 | `hk_vps_4_settings.md` | → `memory.agent-mate.ai/specs/` | ✅ |
| 6 | `vps4_new_deployment_instruction.md` | → `memory.agent-mate.ai/specs/` | ✅ |
| 7 | `mcp_oss_bak_com_requirements.md` | → `memory.agent-mate.ai/specs/`（**2026-09-20 随项目移交移出本仓**） | ✅ 迁移已完成；文件本身此后被移除 |
| 8 | `deploy/`（5 文件） | → `memory.agent-mate.ai/deploy/` | ✅ |
| 9 | `backup/` 脚本 | → `memory.agent-mate.ai/backup/`（dev-plan.md §4 落地时创建） | ☐ |
...
| 真实值归属 | 以上 4 个 IP 与 `<OSS_BUCKET>` / `<QWEN_BASE_URL>` 真实值统一写入 `memory.agent-mate.ai/secrets.local.hk_vps_4.md`（被 `.gitignore` 的 `secrets.local*` 忽略，**永不入仓**）；仓库内一律用占位符 |
...
> **历史处理说明（2026-09-20）：** 野草云4 的 `<VPS4_IP>` 已由公开 DNS 解析（`portainer4.agent-mate.ai` / `nginx4.agent-mate.ai` 均指向它），属「卫生」级别；故**不重写历史**，仅在工作区将真实值迁出到 `memory.agent-mate.ai/secrets.local.hk_vps_4.md` 并改用具名占位符。另 3 个 IP（`<VPS3_IP>` / `<PG_HOST>` / `<MYSQL_HOST>`）未公开解析，是真正有保密价值的，现已从仓库文档移除。
...
| 公开仓意外提交 secrets / 真实 IP | 迁移前逐文件检索；`secret-check.sh`（pre-commit 钩子 + `make secret-check` 手动）长期防线；真实值只在 `memory.agent-mate.ai/secrets.local.hk_vps_4.md` |
...
| 日期 | 决策 |
| --- | --- |
| 2026-09-20 | 确立资产隔离：初版为物理分仓；同日改采**协同布局**（父仓 `memory.agent-mate.ai` 公开仓 + 嵌套 gitignored 上游 clone + `memory.agent-mate.ai/` 自有资产）。**迁移已执行（同日，见 §9 清单）。** |
| 2026-09-20 | 新增**版本契约层**：`memory.agent-mate.ai/upstream.lock`（单一真相源）+ `specs/upstream_coupling_surface.md`（耦合面清单）+ `scripts/upstream-preflight.sh`（升级准入判定）+ `.github/workflows/upstream-track.yml`（每日跟踪）。§3 护栏 5 与 §5 规则 3 的映射载体由 `deployment-plan.md` 正文改为锁文件；§2 判据补充根目录例外（`.github/workflows/`，平台强制）。 |
| 2026-09-20 | **目录更名**：`memory.agent-mate.ai/docs/` → **`memory.agent-mate.ai/specs/`**；仓根 `specs/`（ADR / knowledge）并入 `memory.agent-mate.ai/specs/`。理由：自有资产（含决议与知识）全部集中在 `memory.agent-mate.ai/` 之下，根目录只余基础设施与平台强制路径，§2 判据回到「只有两种归属」。同步清扫全仓路径引用（Makefile / upstream.lock / 预检脚本 / Actions / deploy 文档 / ADR / knowledge）。 |
| 2026-09-20 | **取消「转私有仓」决议，保持公开仓**：把 4 个真实公网 IP（真实值见 `memory.agent-mate.ai/secrets.local.hk_vps_4.md`）从仓库文档迁出到该 gitignored 文件，文档内改用具名占位符（`<VPS4_IP>` / `<VPS3_IP>` / `<PG_HOST>` / `<MYSQL_HOST>`）。新增 `secret-check.sh` 扫描已跟踪/暂存文件公网 IP + pre-commit 钩子（`make hooks-install`）防复发。理由：野草云4 IP 已由公开 DNS 解析，重写历史零收益且需 force push；详见 deployment_strategy.md §0 决议 10 与本文档 §10。 |
