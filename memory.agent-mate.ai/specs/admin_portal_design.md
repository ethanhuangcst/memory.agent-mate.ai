...
# <tag> 从 memory.agent-mate.ai/upstream.lock 的 IMAGE_TAG 注入（构建参数），不手写
```
...
**契约探针设计**：`memory.agent-mate.ai/scripts/upstream-preflight.sh --with-image` 已能拉取镜像校验指纹；应**新增一步**：在候选镜像内执行 `ai-memory --help` / `mcp --help` / `id aimem`，与上述 8 项的 baseline 快照比对，缺项或改名即阻断升级。
...
| 3 | `asset_isolation_plan.md` §2 | 自有资产只有 `memory.agent-mate.ai/` 一个根 | ⚠️ **修订** —— 两个根：`memory.agent-mate.ai/` + `admin_portal/` |
...
| 9 | `tmp_user_key_option1.md`（仓根，未跟踪） | 「每用户 SSH 密钥 + forced command」暂记 | ⚠️ **待你处置**：它是被否路线的留档，但位于**公开仓根目录且未 gitignore**。建议移入 `memory.agent-mate.ai/specs/` 作为附录或加入 `.gitignore` |
...
| 8 | 本设计文档的放置位置 | 若将来出现第二个部署目标 | 现置于 `memory.agent-mate.ai/specs/`（沿用现有约定）；若第二目标出现，再考虑上移为顶层 `specs/` |
