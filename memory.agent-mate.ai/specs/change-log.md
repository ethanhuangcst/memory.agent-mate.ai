...
**改了什么**：新增 [`../probes/container-attack-surface-probe/`](../probes/container-attack-surface-probe/README.md)（三相：相 1 离线判据证明 · 相 2 底座代理 · 相 3 门户镜像实测）；**产品代码 / `deploy/` 一行未动**。

- **判据定档（三条 AC → 两/三层判据）**：`AC13.1` = compose 层零 socket 引用 **且** 容器层不存在 `/var/run/docker.sock`（`TC-P-L1-08` 的容器侧）；`AC13.2` = compose 层零特权键 **且** 容器层 `id -u != 0`；`AC13.3` = ①`read_only` **且**给出可写必要挂载（**只看 `read_only` 会假绿**）②`mem_limit` 与 `pids_limit` **二者齐备** ③包管理能力集合**只剩 `/bin/sh`**（**不能把 `sh` 一起删** —— `healthcheck` 的 `CMD-SHELL` 依赖它）。
- **判据可行性**：`S1`–`S5` 五组正/反对照（样本**全合成**，不拿制品现状当样本 —— 口径同 `#1` 探针 `C5`）；`S5` 的三态（含 apt / 只剩 sh / **空输出**）当场抓出了我自己写反的期望值。
- **现状（机械核实）**：`AC13.1`/`AC13.2` 在 compose 与 Dockerfile 侧**已合规**（socket 0 处 · 特权键 0 处 · `USER 999:999`）；**三处缺口** —— ① 只读根未配齐 ② 资源限额未配齐 ③ **镜像带包管理能力**（底座实测 `/usr/bin/apt-get` `/usr/bin/apt` `/usr/bin/dpkg` ⇒ `AC13.3` ③ **当前不合规**）。
- **不把「已知不合规」写成断言**：`AC13.3` ③ 现在必然红 ⇒ 探针只记 `info`，待施工后由 `Q` 类**契约断言**接手（同一教训本轮已踩一次：`#2` 探针的 `H1`/`V2`）。
...
- **探针自伤修正（同批，两次踩坑）**：① 第 1 批落地后本探针的 `H1`/`V2` **假红** —— 它们把「**制品现状**」当样本（口径同 `#1` 探针 `C5` 的教训：**会随正确实现翻转的观察不该写成断言**）；② 改为往**真 compose** 注入后，因真 compose 已有 `healthcheck` ⇒ **重复键、YAML 非法**（`mapping key "healthcheck" already defined`）。⇒ 样本改为**从合成底座派生**（与制品现状彻底解耦），并新增 **`Q1`/`Q2` 两条编排契约断言**（断言落地后的契约 —— 一旦被改坏就该红，体例同 `#1` 探针的 `C1`–`C10`）。本机复跑 **11 PASS / 0 FAIL / 1 未判**；CI 复跑（run **`36240706277`**）**16 PASS / 0 FAIL / 0 未判**。
...
**改了什么**：状态改标 **`Pending`**，语义 = **本地判据已定型（Sprint 3 `#3`）、尚待生产复验**；**关闭条件 = 在生产卷上通过 `V1` 负向验证**，落点 `#11`（生产卷复验）与 `#14`（上线验收；`#14` 仍是本行执行点）。同批改「RID Registry」`D5` 行与 Sprint 5 收尾句的措辞；`RID 覆盖对照` 的 `D5` 行只有链接、无状态列 ⇒ **无需改**。验证：`make doc-links` 零悬空。

### Sprint 5 `#2` 开工准备：判据可行性探针（只探不造）

**为什么**：按 [`ADR-017`](./adr/ADR-017-complexity-probe-before-real-build.md) 判据未定档前不施工；`#2` 有四处**做法未定**的地方（版本断言的归属、`healthcheck` 的形态与取值、编排侧缺口、一处「登记了但没人读」的键），先证明「判据能不能判」。**产品代码与 `deploy/` 一行未动。**

**改了什么**：新增判据可行性探针 [`../probes/portal-artifact-contract-probe/`](../probes/portal-artifact-contract-probe/README.md)（三相：相 1 离线判据证明 · 相 2 底座镜像代理 · 相 3 门户镜像实测）。
...
**验证**：探针 **9 PASS / 0 FAIL / 1 未判 · `rc=30` · 两跑一致**；相 3 骨架另用临时靶镜像冒烟（`T1` PASS · `T5` 对无 LABEL 镜像**转红** = 敏感性证明 · `T2`/`T4` 按预期 FAIL · `T3` 实测 `RestartCount=7`），靶镜像与临时目录**跑完即删** · `make doc-links` 零悬空 · `make deploy-doc-audit` 9/0 退 0 · `make secret-check` ✓ · `git diff --check` 干净。

**边界**：**不改产品代码 / `deploy/` / `scripts/`**（探针的所有改写只发生在 `mktemp -d` 的副本里）· **`#2` 未开工**（待两项拍板）· `PORTAL_ROOT` 的处置**只登记、未改**（归 `#2` 施工）· **未**给 `deploy-doc-audit` 补双向判据（只登记方向缺口）。
