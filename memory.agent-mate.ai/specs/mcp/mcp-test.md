...
> **`3.7` 的自动化落点（2026-09-24，开工前置；交付时回填实测证据）**：`3.7`「跨用户隔离」的两条验收条件 —— 「A/B **经同一门户实例**互不可见」与「**两侧均记录解析后的库路径**」—— 既有取证**都有缺口**（覆盖核对结论）：① `scripts/iso-probe.sh`（Sprint 3，`V2` / `V3` / `V4` 本地版）已证**上游级**隔离（交叉 `get` / 库路径解析 / 共享主库不变），但那是用**显式 env 直连上游**、**没有经过门户**；② `make portal-mcp-session-probe`（`3.3` / `3.4`）虽经门户起了两个用户的会话，但验的是**进程与文件隔离**，**从没让 B 去召回 A 写的东西** ⇒ 而「令牌 → handle → 库路径」这一步**恰恰在门户**（`R1` / `R2` 指向的就是它）。判据形状已由 `3.15` 探针（[`../../probes/cross-user-probe/`](../../probes/cross-user-probe/)）**实测钉死**（**7/7 断言 PASS、退出码 `0`、两轮复跑一致**）：`TC-M-L1-03`（双向互不可见，**同一门户实例**；B 召回 A 的标记实测 **`count:0`**）· `TC-M-L1-04`（按 id 直取 ⇒ 上游回 **`memory not found`**，「按 id 直取」的工具是 **`memory_get`**）· `AC4.3` 的机制（`3.4` 交付）在**跨用户**场景下成立（门户 `audit` 表两侧各一行 `mcp_session_opened`，`detail_json.dbPath` 各自指向 `/data/users/<自己>/ai-memory.db`）。**硬约束**：召回类判据一律按「**回包里有没有那个标记**」判（`3.14` 的教训），**且必须带灵敏度对照**（B 能召回**自己**写的 ⇒ 否则「B 检索不可用」会让判据假绿）。
...
| 退出码 | `0` 全过 · `10` 前置不足 · `20` 门户起不来 · `30` 断言失败（与 [`../../scripts/portal-mcp-probe.sh`](../../scripts/portal-mcp-probe.sh) 同范式；经 `make` 时非零码会被压平为 `2`，脚本头已登记） |
