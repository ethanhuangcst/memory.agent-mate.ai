...
| TC-M-L1-14 | **失败面逐场景状态码与错误体** | 按 [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的表逐行核对：未携带 / 无效 / 已吊销 / 非 `memo_` 前缀 → **401** · 面不匹配 → **403** · spawn 前断言失败 → **500** · spawn 失败 → **503** · 上游超时 → **504** · 配额超限 → **429** `QUOTA_EXCEEDED` · 客户端断开 → 仅回收无响应；且错误体**不含**内部路径 / 堆栈 / 上游 stderr 原文 | `MS1 AC-M1.2` · `AC-M1.4` · `MS4 AC-M4.3` · `AC4.4` · `AC7.1` | Sprint 4 |
