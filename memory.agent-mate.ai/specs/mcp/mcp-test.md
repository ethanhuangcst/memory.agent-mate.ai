...
| L1.5 | 隔离探针 | [`../../scripts/probes/iso-probe.sh`](../../scripts/probes/iso-probe.sh)（A/B/C 组，P1a–P6） | **R1 / D5**：漏设 `AI_MEMORY_DB` 必须 fail-loud、解析目标非共享主库且共享主库不变；显式用户库双向隔离（[`./mcp-design.md`](./mcp-design.md) §6） |
| L1.6 | 多语言探针 | [`../../scripts/probes/i18n-probe.sh`](../../scripts/probes/i18n-probe.sh)（三语言写入 → 关键词 12 组对照 → 语义召回 + 按 id 直取 → 结论矩阵；退出码 0/10/20/30/40/50，`I18N_PROBE_STRICT=1` 把语言边界当断言） | 对 CJK 记忆**误用关键词通路** → 静默 count=0 假象「记忆丢了」（结论矩阵与用例见 §4-E） |
| L1.7 | 配额探针 | [`../../scripts/probes/limits-probe.sh`](../../scripts/probes/limits-probe.sh)（一次性库 `/data/users/limits-probe/` + 每轮全新身份 + **仅 env 注入**小阈值；退出码 0/10/20/30/40/50/60/70，`--self-test` 负向自测） | 上游 `[limits]` **改配置不等于行为生效**：配额逐 `(agent_id, namespace)` 盖章、且 **CLI 一次性写入不计费**，用错验证方式会得出「配额失效」的假结论（用例见 §4-D TC-LIMIT） |
| L1.8 | 每库维护探针 | [`../../scripts/probes/gc-probe.sh`](../../scripts/probes/gc-probe.sh)（独立一次性库 + 静态审计维护脚本 + `--self-test`；退出码 0/10/20/30/40/50/60/70） | 上游维护覆盖面判错：**TTL 驱逐不只由 `gc` 触发**（`store`/`list`/`recall`/`import` 都会惰性清扫）⇒ 误把 `gc` 计数当「过期总量」；漏传 `--db` ⇒ 静默新建相对路径库或误操作主库；v0.11 attestation 缺省翻转后维护任务升级即失效（用例见 §4-C TC-GC） |
...
| TC-TIER-01 | `initialize` + `tools/list` 计核，逐档核对 | core=8 / graph=20 / admin=22 / power=57 / full=101 | **已完成 2026-09-21**（探针 [`../../scripts/probes/profile-probe.sh`](../../scripts/probes/profile-probe.sh)：7 档独立进程 + 默认档对照 + `core,lifecycle`=14，退出码 0；四处模板均已写 `--profile`，用户 `core` / 管理员 `admin`） |
...
> 现役探针：[`../../scripts/probes/iso-probe.sh`](../../scripts/probes/iso-probe.sh)（退出码 0 全通过 / 10 前置 / 20 解析链 / 30 隔离 / 40 维护 / 50 方案②会话），组 A=P1a/P1b/P4，组 B=P2/P3/P5，组 C=P6。可重复性已验证：第二次起必然命中 near-duplicate 去重，探针**分会话**处理（写入会话先取「生效标记」再另开会话检索），故重复运行稳定。
...
现役探针：[`../../scripts/probes/i18n-probe.sh`](../../scripts/probes/i18n-probe.sh)（无参数；隔离库 `/data/users/i18n-probe/`，不碰主库与 iso 库；`I18N_PROBE_STRICT=1` 把语言边界按本表登记值断言，防上游行为漂移；退出码 0/10/20/30/40/50）。源码依据与契约面：[`./mcp-design.md`](./mcp-design.md) §2「记忆内容多语言」/ §9 J4。
...
| TC-M-L1-02 | 无令牌 / 已吊销令牌 / 用户已停用令牌连接 | 会话建立**被拒**且不降级为匿名：**HTTP 401** + `{"error":"unauthorized"}`（「不存在」与「已吊销」**同形**，不给探测者区分信号） | `MS1 AC-M1.2` | Sprint 4 |
...
| TC-M-L3-02 | **负向门禁**：`AI_MEMORY_DB` 置空 / 指向他用户有效库 | spawn **前**拒绝并告警：**HTTP 500** + `{"error":"spawn_assertion_failed"}`；不出现静默落主库的会话；会话未写入数据；**必写审计行** | `MS4 AC-M4.3` · `AC-M4.4` | Sprint 4（对应 V1 / D1） |
...
| TC-M-L1-14 | **失败面逐场景状态码与错误体** | 按 [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的表逐行核对：未携带 / 无效 / 已吊销 / 非 `memo_` 前缀 → **401** · 面不匹配 → **403** · spawn 前断言失败 → **500** · spawn 失败 → **503** · **上游超时 → 视时点而定**（握手期 **503** / 转发期 **MCP 层错误** / 转发阶段同步异常 **504**）· **转发阶段的其他异常 → 502 `upstream_error`** · 配额超限 → **429** `QUOTA_EXCEEDED` · 客户端断开 → 仅回收无响应；且错误体**不含**内部路径 / 堆栈 / 上游 stderr 原文 | `MS1 AC-M1.2` · `AC-M1.4` · `MS4 AC-M4.3` · `AC4.4` · `AC7.1` | Sprint 4 |
| TC-M-L1-15 | **未注册的请求原样转发** | 桥未显式注册的方法经 `/mcp` 调用时，客户端收到的是**上游的**应答或**上游自己的** `-32601`（与直连上游一致），**不是**桥 / SDK 合成的 `Method not found`；结果形状由上游决定（桥**不校验**，宽松 schema 透传） | `MS1 AC-M1.1` · `MS3 AC-M3.1` | Sprint 4 `3.8` |
...
| TC-M-L1-18 | **上游身份与能力透传到客户端** | 客户端 `initialize` 后拿到的是**上游的** `serverInfo`（`name` / `version` / `title` / `websiteUrl` / `description` 字段与取值全保留）与 `instructions`；`capabilities` 为**经 SDK schema 归一化后的**上游能力 —— 已知键保留（`experimental` / `logging` / `prompts` / `resources` / `tools` …）· **未知键被丢弃**。判据写「**字段与取值**」，**不写「逐字节」**（键序会被 schema 解析重建） | `MS1 AC-M1.1` · [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的「桥对客户端的身份与能力」 | Sprint 4 `3.9` |
| TC-M-L1-19 | **超时分层：握手超时与请求超时互不牵连** | 把握手超时与请求超时**故意反着设**（短握手 / 长请求），**耗时超过握手超时**的调用仍成功；请求超时则照旧以 **MCP 层错误 `-32001`** 返回 ⇒ 两个值各管一段。判定必须**同时断言耗时**（只断状态码会漏掉「超时没生效」） | `MS1 AC-M1.2` · [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的「超时分层」 | Sprint 4 `3.9` |
| TC-M-L1-20 | **桥不吞掉上游的请求：`logging/setLevel` 抵达上游** | 上游声明 `logging` 时，客户端的 `setLevel` **必须抵达上游**（判据以上游侧**传输层留痕**为准，**不以**「上游返回了什么」为准 —— SDK 在上游同样会本地处理它）。反证：不把 SDK 自动注册的 `logging/setLevel` handler 移除时，上游**收不到** | `MS3 AC-M3.1` | Sprint 4 `3.9` |
| TC-M-L1-21 | **两会话并发 ⇒ 两个独立上游子进程**（真上游） | 两个不同令牌的并发会话在容器内对应**恰好两个**上游进程且 **PID 互不相同**；各自进程的 `AI_MEMORY_DB` / `AI_MEMORY_AGENT_ID` 指向**自己用户**的库与身份。判据以**容器内 `cmdline` 计数 + `/proc/<pid>/environ`** 为准（容器**无 `ps`**；Rosetta 下 `/proc/<pid>/exe` 恒指向 rosetta ⇒ 只能按 cmdline 精确前缀，且须带 `--profile core`）；`environ` **用同 uid 读**（`-u 0` 反而 `Permission denied`）。**同用户重开会话必须是新 PID**（禁池化） | `AC4.6`（[`../web-portal/web-stories.md`](../web-portal/web-stories.md)）· `MS3 AC-M3.1` | Sprint 4 `3.3` |
| TC-M-L1-22 | **会话前后无他人痕迹**（真上游） | 会话**真写一次**（`memory_store` ⇒ 走真 embedding）后：① 活跃用户目录**之外零新增文件**；② 用户目录之外**唯一允许变化**的是上游**共享**审计日志 `/data/.local/state/ai-memory/audit/*.jsonl`（实测豁免口径，**不能**写成「`/data` 零变化」）；③ **不参与**本次会话的既有用户（旁观者）其库**零变化**；④ 收尾后无临时文件残留 | `AC4.5`（[`../web-portal/web-stories.md`](../web-portal/web-stories.md)） | Sprint 4 `3.3` |
| TC-M-L1-23 | **全链路：写入落**用户库**且「新会话」可召回**（真上游） | 会话 A 写唯一标记 → **收尾** → 会话 B（**另一个进程**）用同一标记召回 ⇒ 回包**含该标记**（证明写入**真的落进用户库**，而不是留在该进程内存里）；**不优雅收尾**（直接 `close()`，不 `terminateSession`）后同样成立 ⇒ 判据**不**依赖「先 terminate」。**判据必须按「回包文本里有没有该标记」判，禁止按 `memory_recall` 的 `count` 判** —— 它是**语义混合检索**，`count` = **返回条数**：从未写入的标记同样会有相关命中（⇒ `count:0` 永不出现），库里多一条时 `count:1` 会变 `count:2` ⇒ 按 `count` 写的断言**必然假失败**；且必须带**反向对照**（「从未写入的标记不出现在回包里」）以防空转 | `3.6` 的验收条件（一次写入召回跑通）· `MS1 AC-M1.1` 的**真版** | Sprint 4 `3.6` |
| TC-M-L3-05 | **负向**：跨用户接管被拒且**不伤及**受害者 | 用甲令牌 + 乙的 `mcp-session-id` 请求 ⇒ **401** 且**不新增**子进程；**且**乙用**自己的**令牌 + 原 `sessionId` **仍可用**、其子进程仍受管 ⇒ 收尾后归零。**当前不成立**（`3.12` 探针实测缺陷：`route.ts` 在该分支上 `registry.remove()`，把**第三方**会话从注册表摘掉 ⇒ 乙随即 401、其子进程成孤儿直到门户退出） | `AC4.6` 的反方向（「不存在把同一子进程复用给不同用户的路径」） | Sprint 4 `3.3`（**缺陷修复随本行**） |

> **`3.7` 的自动化落点（2026-09-24，**已交付**，实测已回填）**：`3.7`「跨用户隔离」的两条验收条件 —— 「A/B **经同一门户实例**互不可见」与「**两侧均记录解析后的库路径**」—— 既有取证**都有缺口**（覆盖核对结论）：① `scripts/probes/iso-probe.sh`（Sprint 3，`V2` / `V3` / `V4` 本地版）已证**上游级**隔离（交叉 `get` / 库路径解析 / 共享主库不变），但那是用**显式 env 直连上游**、**没有经过门户**；② `make portal-mcp-session-probe`（`3.3` / `3.4`）虽经门户起了两个用户的会话，但验的是**进程与文件隔离**，**从没让 B 去召回 A 写的东西** ⇒ 而「令牌 → handle → 库路径」这一步**恰恰在门户**（`R1` / `R2` 指向的就是它）。判据形状已由 `3.15` 探针（[`../../probes/cross-user-probe/`](../../probes/cross-user-probe/)）**实测钉死**（**7/7 断言 PASS、退出码 `0`、两轮复跑一致**）：`TC-M-L1-03`（双向互不可见，**同一门户实例**；B 召回 A 的标记实测 **`count:0`**）· `TC-M-L1-04`（按 id 直取 ⇒ 上游回 **`memory not found`**，「按 id 直取」的工具是 **`memory_get`**）· `AC4.3` 的机制（`3.4` 交付）在**跨用户**场景下成立（门户 `audit` 表两侧各一行 `mcp_session_opened`，`detail_json.dbPath` 各自指向 `/data/users/<自己>/ai-memory.db`）。**硬约束**：召回类判据一律按「**回包里有没有那个标记**」判（`3.14` 的教训），**且必须带灵敏度对照**（B 能召回**自己**写的 ⇒ 否则「B 检索不可用」会让判据假绿）。**已交付（2026-09-24）**：落在**扩展既有 `make portal-mcp-session-probe`**（用户拍板 G-a ①），新增四条判据 **`TC-M-L1-23·1`–`·4`** ⇒ 断言数 **11 → 15 全 PASS、退出码 `0`**（改前基线 **11/11**；既有 11 条零回归）。**判据敏感性已证明（不是空转）**：临时把 B 的令牌换成 A 的（模拟「令牌 → handle → 库路径」**串号**这一失效形态）⇒ 新增四条**全部转红**（`23·1` 抓到 B←A 含标记 · `23·4` 显示 B 的审计行缺失），还原后转绿。**顺带修一处门禁缺陷**：四个真上游脚本的 `cleanup()` 把 `local rc=$?` 放在 `if` 之后 ⇒ `$?` 被 `if` 自身（条件为假 ⇒ `0`）覆盖 ⇒ **退出码恒为 `0`**、文档承诺的 `10 / 20 / 30` **根本传不出来**（门禁在 CI 里永不红）；已把捕获提到函数第一行，并以**合成对照**钉死（旧写法 ⇒ `0` · 新写法 ⇒ `10`）。

> **`3.6` 的自动化落点（2026-09-24，**已交付**，实测已回填）**：`3.6`「全链路联通」的两条验收条件**已被既有产物覆盖大半** —— ① 「一次写入召回跑通」：`make portal-mcp-probe`（`3.1` 交付）断言「工具数 / 写入成功 / 召回命中 / 库落该用户目录」，但都在**同一会话内** ⇒ 缺「**换一个进程仍能召回**」那半条（「真的落进用户库」）；该半条已由 `3.14` 探针实测**成立**，判据登记为 `TC-M-L1-23`。② 「拒绝路径按预期失败」：**无令牌** → `TC-M-L1-02`（离线集成 + 门户侧均已验）· **越权库路径** → `3.2` 的 spawn 前断言（**fail-closed，发生在 spawn 之前** ⇒ 判据本质上在**离线层**，真上游**无观测面**，如实登记、不硬造真上游断言）。⇒ **本行的实质 = 把「跨进程召回」这条判决据挂到门户链路，并复跑收口**（代码排布见计划文件 `.codebuddy/plans/sprint4-3_6-full-chain.md`）。**另含一处顺手加固**：`portal-mcp-probe.sh` 的召回断言按 `count:1` 写（现在稳、但脆）⇒ 改为按标记判。**已交付（2026-09-24）**：① `admin_portal/tests/fixtures/probe-runner.mts` 新增两条断言 —— **「新会话（另一个子进程）仍能召回 ⇒ 写入真的落进了该用户库」** + **反向对照**「从未写入的标记不出现在回包里」；既有召回断言由 `count:1` 改为**按标记判**；② `make portal-mcp-probe` 由 **4 项 → 6 项断言全 PASS、退出码 `0`**（改前基线 **4/4**）；③ **判据敏感性已证明（不是空转）**：临时删掉该用户的库后重测，**同一判据由 PASS 转 FAIL**（库在 ⇒ PASS · 库删 ⇒ FAIL）；④ 顺手修该脚本两处会误导人的前置提示（`make local-up` 目标不存在 · `cd admin_portal` 相对路径不成立）。

> **`3.3` 的自动化落点（2026-09-24 交付，实测已回填）**：分两层，**判据形状已由 `3.12` 探针预先钉死**（[`../../probes/session-isolation-probe/`](../../probes/session-isolation-probe/)，**8/8 断言 PASS、退出码 `0`、两次复跑一致**，另实测到 2 处缺陷）。① **机制层（离线、可重复）**：`admin_portal/tests/integration/mcp-bridge.test.ts` 的「会话隔离（3.3：TC-M-L3-05）」describe —— 假上游的 `--env-dump` **逐进程落盘**（文件名含 PID）⇒ **数文件 = 数 spawn**，「接管被拒不新增进程」可用它判；`--closed-marker` 判「子进程确已退出」。反向（缺陷）用例也在这里：接管被拒后，**受害者**用自己令牌 + 原 `sessionId` 必须仍可用。② **真上游端到端**：`TC-M-L1-21` / `TC-M-L1-22` 的判据**天然需要真上游**（假上游既不是一个 `ai-memory` 进程、也没有用户的库文件）⇒ 走 §4-G 的跑法。**③ 开工准备阶段可先跑探针本身**（同一套观测、无需新代码）：`bash memory.agent-mate.ai/probes/session-isolation-probe/probe.sh`。**④ 实测落点（交付）**：**离线支** = `admin_portal/tests/integration/mcp-bridge-session-isolation.test.ts` 的**三支**（`make portal-test` **322 passed / 30 files**）—— 「两个并发会话各自 spawn 一个上游进程，pid 互不相同且各带自己的库与身份」（`TC-M-L1-21`）· 「同用户重开会话**不复用**旧进程（禁池化）」（`TC-M-L1-21`）· 「跨用户接管被拒 **401** 且**不伤及**受害者：不新增进程、**受害者仍可用**、其子进程仍受管」（`TC-M-L3-05`）。**真上游支** = `make portal-mcp-session-probe`（§4-G 的跑法，**9/9 断言 PASS、退出码 `0`**）：容器内进程 `0 → 2` 且 PID 互不相同、各自 `AI_MEMORY_DB` / `AI_MEMORY_AGENT_ID` 正确 · 同用户重开为新 PID · 用户目录之外**零新增**（唯一变化 = 上游**共享**审计日志）· 旁观者用户库**零变化** · 接管 **401** 且不新增进程 · **受害者仍可用** · 收尾后**无孤儿**。**判据敏感性已证明**（不是空转）：临时回退 `route.ts` 的修正后重跑，**只有 `TC-M-L3-05` 那一支转红**（其余 321 项仍绿），恢复后转绿。**探针回归**：`3.12` 探针的两条缺陷发现已转为正面结论（`[i] 「不可侵扰」成立` / `[i] 收尾后无孤儿`）。**如实登记**：`TC-M-L1-22` **没有离线支** —— 假上游既不是一个 `ai-memory` 进程、也没有用户的库文件，该条只在真上游层判。

> **`3.2` 的自动化落点（2026-09-24）**：判定逻辑抽成**导出纯函数**后分为两层 —— 单测（`admin_portal/tests/unit/user-db-path.test.ts`，**6 支**）覆盖**五支判定**：① `empty`（`AI_MEMORY_DB` 置空 / 缺键；模板是内建常量 ⇒ **集成层不可达**）② `outside_users_root`（`../bob` / `../../x` / `/tmp/alice/…`）③ `handle_mismatch`（handle 合法、路径指向**他人**库）④ `invalid_handle`（`''` / `Alice!` / 33 字符 —— 决策修订新增，同样到不了 HTTP 层）⑤ 正向对照；外加一条「抛出的错误上挂 `reason` 且消息不含路径」。**端到端**（`admin_portal/tests/integration/mcp-bridge.test.ts`）覆盖可达的那一支：用 `insertUser`（repo 层，绕过 `createUser` 的 `validateHandle`）插 `handle='alice/../bob'` → `issueKey` 签令牌（`userDirectory` 对该 handle 放行 ⇒ **真的能到达 spawn**）→ 打 `/mcp` ⇒ 断言 **HTTP 500** + `{"error":"spawn_assertion_failed"}` + 审计行（`stage: 'spawn_assertion'`、`reason` 为 **`invalid_handle`**（判定**先校验 handle**，含 `/` 的输入在路径运算前就被拦下）、**且 `detail_json` 不含 `/data/users/`**）。**「未 spawn」的判据用标记脚本**（`tests/fixtures/marker-command.mjs` 作 `PORTAL_LAUNCH_OVERRIDE`）：它**被执行的第一个动作就是 append 标记** ⇒「标记文件不存在」**直接等于**「命令从未被执行」，无时序假设；比 `--env-dump` 强（后者无法区分「没起」与「起了但在 dump 前退出」）。该判据同时覆盖「未写入任何数据」与「不落共享主库」。**越界类与空值类到不了 HTTP 层**：`userDirectory` 对它们抛错 ⇒ `issueKey` 返回 `directory-failed`、令牌签不出来。

> **`3.9` 的自动化落点（2026-09-24）**：三条全部落在 `admin_portal/tests/integration/mcp-bridge.test.ts` 的 `上游身份与能力透传、超时分层（3.9：TC-M-L1-18 / 19 / 20）` describe —— `TC-M-L1-18` = 起**两个**独立实例：带 `--identity` / `--capabilities` / `--instructions` 的那条断言三者**等于上游声明值**（能力刻意取一组与旧硬编码**不同**的值 ⇒ 能区分「透传」与「照抄」）；**不带**任何开关的那条断言 `getInstructions()` 为 `undefined`，反证网关**不凭空造字段**。`TC-M-L1-19` = 同一夹具形态（`--tool-hang-ms=1500`）下按**反着设**的两个超时各起一个实例：短握手（300）+ 长请求（5000）**必须成功**且耗时 ≥1500ms；长握手（5000）+ 短请求（300）**必须在 `-32001` 处失败**且耗时 <1500ms（错误码与耗时**一起断**）。`TC-M-L1-20` = 夹具以 `--capabilities` 声明 `logging`，并用新增的 `--request-log` 在**上游侧传输层**留痕，判据取该留痕而非上游返回值。**夹具新增五个观测位**：`--identity=NAME@VERSION` · `--capabilities=<json>` · `--instructions=<text>` · `--tool-hang-ms=N` · `--request-log=PATH`（默认值全部保持原状 ⇒ 既有用例零回归）。**反证无法在集成层构造**（「网关不拽回 `logging/setLevel`」这个状态在产品代码里不存在），由 `probes/bridge-identity-probe/` 断言 4 给出。
...
### G. 真上游端到端（online）的跑法与判据（Sprint 4 `3.3` 开工前置定档，2026-09-24）

> **为什么单列一节**：本文件的用例判据此前默认「离线可重复」，而 `AC4.5` / `AC4.6` 的两条判据 —— **容器内进程数**与**容器内文件足迹** —— **天然需要真上游**：假上游既不是「一个 `ai-memory` 进程」，也没有「用户的库文件」。不定跑法，这两条就永远只是纸面判据；`3.9` 那轮已经出现过一次同类问题（「真上游上未实测的断言会让脚本不稳定」，故当时**不**往端到端脚本里加）。
>
> **本轮（开工准备）就能跑**，不需要新代码：`bash memory.agent-mate.ai/probes/session-isolation-probe/probe.sh`（`3.12` 探针，同一套观测手法，**8/8 断言 PASS、两次复跑一致**）。**产品侧入口** `make portal-mcp-session-probe` 由 `3.3` 交付（把探针的观测固化成脚本 + runner，见 §4-F 的 `3.3` 落点）。

| 项 | 定档 |
|---|---|
| 入口 | `make portal-mcp-session-probe`（`3.3` 新增；`PROBE_PORT=` / `PROBE_CONTAINER=` 可覆盖）—— 与 `make portal-mcp-probe`（`3.1` 的单会话真链路）**并存**，不扩改后者（既有门禁不失稳） |
| 前置 | 容器常驻（`make local-up`）· 门户依赖已装 · 端口空闲 · **docker endpoint 可取**。缺任一 ⇒ **退出码 `10`**（响亮失败，不伪装通过） |
| 外部依赖 | docker（含 endpoint：模板把 `HOME` 固定为 `/data`，不显式给 endpoint 时 docker CLI 按 `$HOME` 找不到 socket）· 上游 key（宿主 `DASHSCOPE_API_KEY`，缺则从容器里取）—— `memory_store` 那条**没有降级余地**，故缺 key 视为前置不足 |
| 观测面 | ① 容器内 **`cmdline` 计数**（**无 `ps`**；Rosetta 下 `/proc/<pid>/exe` 恒指向 rosetta ⇒ 只能按 cmdline 精确前缀，且前缀**必须带 `--profile core`**，否则会把别处遗留的同形孤儿一起数进来）② **同 uid** 读 `/proc/<pid>/environ`（判「以**谁的**库与身份启动」；`-u 0` **读不到**，实测）③ `find /data /tmp -type f -printf '%p\|%T@\|%s\n'` **前后快照差分** |
| 拓扑无关 | 观测参数化为「**容器名 + cmdline 前缀**」⇒ 本机借壳（门户在宿主 + `PORTAL_LAUNCH_OVERRIDE` 走 `docker exec`）与生产 β′（门户在容器内直接 spawn）用**同一套判据**，不把判据绑在 `docker exec` 这一形态上 |
| 退出码 | `0` 全过 · `10` 前置不足 · `20` 门户起不来 · `30` 断言失败（与 [`../../scripts/probes/portal-mcp-probe.sh`](../../scripts/probes/portal-mcp-probe.sh) 同范式；经 `make` 时非零码会被压平为 `2`，脚本头已登记）。**2026-09-24 修一处缺陷**：此前 `cleanup()` 把 `local rc=$?` 放在 `if` 之后 ⇒ `$?` 被 `if`（条件为假 ⇒ `0`）覆盖 ⇒ **实际恒为 `0`**，上述非零码**根本传不出来**（门禁在 CI 里永不红）；现捕获在函数第一行，四个真上游脚本（`3.1` / `3.3` / `3.12` / `3.15` 的）同批修 |
| 原始证据 | 断言明细落 `out/probe-<时间戳>.log`（探针）与 `/tmp/portal-mcp-session-probe-*.log`（`3.3` 脚本）；**均不入库** |
| 收尾 | kill 门户后**必须复验**：端口已释放 + 容器内进程数归零（`3.12` 探针把它作为最后一条断言 A10 —— 门户一退出，孤儿也随管道断开消失，实测每次均 `0`）。容器内**预建**的测试用户目录按名清单删除，**不碰**其它用户 |
| 本机局限（如实登记） | 模板库路径在**容器内**解析，而门户跑在宿主 ⇒ 脚本会在容器内**预建**测试用户目录；生产 β′（门户在容器内 spawn）**没有**这个错位 |
...
| 2026-09-21 | **新增 L1.6 多语言探针层与 §4-E 用例（TC-I18N-01..06，Sprint 2 #8）**：三语言 × 三通路结论矩阵实测全通过（存储 / 语义召回 / 按 id 直取支持；关键词仅完整词元、词元内子串与简繁交叉不命中）；§1「检索工具分工」原则升级为精确边界；§2 登记 Sprint 2 #8 完成；§4-D TC-I18N-01 占位归并至 §4-E。探针 [`../../scripts/probes/i18n-probe.sh`](../../scripts/probes/i18n-probe.sh)，结论回写 [`../product-backlog.md`](../product-backlog.md) #10 |
| 2026-09-21 | **档位定档 + 模板落盘（Sprint 2 #9 收尾）**：§2 Sprint 3 #2 提前完成；§3 本地基线 `args` 显式 `--profile core`（生产条不写 —— 由服务端 forced command 决定），并注明「不传时默认也是 core 且**不报错**」；§4-C TC-TIER-01/02 标为已完成（探针 [`../../scripts/probes/profile-probe.sh`](../../scripts/probes/profile-probe.sh)：7 档独立进程 + 默认档对照 + `core,lifecycle`=14，退出码 0），新增 TC-TIER-03（对外模板档位核对：门户 `core` / SSH 用户行 `core` / 主人行 `admin`）；新增面向最终用户的能力文档 [`./mcp-capabilities.md`](./mcp-capabilities.md)（工具功能说明的底稿取自 `memory_capabilities` verbose drilldown） |
...
| 2026-09-23 | **新增 `TC-M-L1-15` / `TC-M-L1-16` / `TC-M-L1-17`（`3.8` 的用例），并修正 `TC-M-L1-14` 的超时口径**：① 三条新用例覆盖 `3.8` 的三条判据 —— **未注册请求原样转发**（收到上游应答或上游自己的 `-32601`，不是桥/SDK 合成的 `Method not found`）· **未注册通知含取消**（`cancelled` 必须**显式注册**才能转发；判据以**上游进程的 stdin 留痕**为准，不以「上游是否处理」为准）· **转发阶段失败分类与诊断**（上游业务错误由 SDK 自动透传且 `code` 原样保留；内部异常按 `-32001` → 504 · `-32000` → 503 · 其余 → 502 `upstream_error`；每类留日志 + 审计；握手失败时 `client` 与 `transport` 双关）。② **修正 `TC-M-L1-14`**：其原判据写「上游超时 → **504**」，与 [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的实测三分支（握手期 503 / 转发期 MCP 层错误 / 同步异常 504）冲突，改指该节并补上 `502 upstream_error` 行。③ **定档依据**：`3.10` 探针（[`../../probes/bridge-fallback-probe/`](../../probes/bridge-fallback-probe/)）五项断言全 PASS、退出码 `0`、两次复跑一致；结论同时回写 [`mcp-design.md`](./mcp-design.md) §5.6.2 的实证块。
| 2026-09-24 | **补记：`3.9` 的用例登记（`TC-M-L1-18` / `TC-M-L1-19` / `TC-M-L1-20`）** —— 该轮已在 §4-F 登记三条用例与自动化落点（上游身份与能力透传 · 超时分层互不牵连 · `setLevel` 抵达上游），但**漏记本表**。本条按「本表登记每一次改动」的体例补记；用例内容、判据与落点未改。依据：`3.11` 探针（[`../../probes/bridge-identity-probe/`](../../probes/bridge-identity-probe/)）**七项断言全 PASS**。 |
| 2026-09-24 | **`3.3` 开工前置：新增 `TC-M-L1-21` / `TC-M-L1-22` / `TC-M-L3-05`，新增 §4-G「真上游端到端（online）的跑法与判据」** —— ① 三条用例覆盖 `AC4.6`（两会话 ⇒ 两进程、PID 互不相同、各自带**自己用户的**库路径与身份；同用户重开必须**新 PID**）· `AC4.5`（活跃用户目录之外**零新增**、**唯一豁免**上游**共享**审计日志 `/data/.local/state/ai-memory/audit/*.jsonl`、旁观者用户库**零变化**）· `AC4.6` 的**反方向**（跨用户接管被拒 **401** 且**不伤及**受害者会话与其子进程、**不新增**进程）；② §4-G 定档真上游 e2e 的入口 / 前置 / 外部依赖 / 观测面 / **拓扑无关**（容器名 + cmdline 前缀）/ 退出码 / 收尾复验 / 本机局限，并写明**本轮即可用探针跑**（[`../../probes/session-isolation-probe/probe.sh`](../../probes/session-isolation-probe/probe.sh)）；③ **定档依据**：`3.12` 探针（[`../../probes/session-isolation-probe/`](../../probes/session-isolation-probe/)）**8/8 断言 PASS、退出码 `0`、两次复跑一致**，另实测到 **2 处缺陷**（接管被拒时把**第三方**会话从注册表摘掉 · 其子进程成孤儿）—— 已分别落为 `TC-M-L3-05` 的判据与 [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的修正项；④ **不新造门禁**：产品侧入口 `make portal-mcp-session-probe` 由 `3.3` 交付，既有 `make portal-mcp-probe` **不动**。 |
| 2026-09-24 | **`3.3`「mcp:一会话一子进程」交付：三条用例转绿，并修掉一处实测缺陷** —— ① **实测落点已回填**（§4-F 的 `3.3` 落点块第 ④ 项）：**离线支 3 支**（`admin_portal/tests/integration/mcp-bridge-session-isolation.test.ts`，`make portal-test` **322 passed / 30 files**）· **真上游支** `make portal-mcp-session-probe`（**9/9 断言 PASS、退出码 `0`**）；② **§4-G 的跑法已落地**（该节原写「产品侧入口由 `3.3` 交付」，现入口已存在，命令与退出码契约不变）；③ **判据敏感性已证明**（不是空转）：临时回退 `route.ts` 的修正后重跑，**只有 `TC-M-L3-05` 转红**，恢复后转绿；④ **修正一处同源缺陷**（`3.12` 探针实测）：跨用户接管被拒时**不得**把**第三方**的会话从注册表摘掉（否则受害者随即 401、其子进程成孤儿）⇒ 会话归属校验拆为「**只拒不动**」与「**自己的传输已关闭才幂等 `close()`**」两支，契约见 [`../web-portal/web-design.md`](../web-portal/web-design.md) §12.5 的「会话归属校验的不可侵扰」；⑤ `3.12` 探针的两条 `[F] 发现` 转为 `[i]` 正面结论；⑥ **既有 `make portal-mcp-probe` 退出码 `0`（零回归）**。 |
| 2026-09-24 | **`3.6`「mcp:全链路联通」交付**：① **判决据补齐**（`TC-M-L1-23`，落在 `make portal-mcp-probe`）—— 新增「**新会话（另一个子进程）仍能召回 ⇒ 写入真的落进了该用户库**」与**反向对照**「从未写入的标记不出现在回包里」两条断言；既有召回断言由 `count:1` 改为**按标记判**（理由见 [`mcp-design.md`](./mcp-design.md) §5.6.2 的 `3.14` 实证块：该工具是**语义混合检索**，`count` = 返回条数）。⇒ **4 项 → 6 项断言全 PASS、退出码 `0`**（改前基线 4/4）。② **判据敏感性已证明**：临时删库后重测，**同一判据由 PASS 转 FAIL**。③ **覆盖核对结论登记**：「拒绝路径」的**越权库路径**判据在**离线层**（`3.2` 的 spawn 前断言 fail-closed），真上游**无观测面** ⇒ 本行**不**硬造真上游断言。④ 顺手修 `portal-mcp-probe.sh` 两处会误导人的前置提示。 |
| 2026-09-24 | **`3.7` 开工前置：`3.15` 探针定档「跨用户隔离」的判据口径** —— ① **「经同一门户实例」的跨用户不可见**（`TC-M-L1-03`：实测 B 召回 A 的标记得 **`count:0`**；反向亦然 ⇒ **双向**）; ② **按 id 直取**（`TC-M-L1-04`：B 用 A 的记忆 id 调 **`memory_get`** ⇒ 上游回 **`memory not found`**）；③ **两侧审计行**各记自己的解析后库路径（`AC4.3` 的机制由 `3.4` 交付，本探针判它在**跨用户**场景下成立）；④ 登记两条**硬约束**：召回类判据按「**回包里有没有那个标记**」判（**禁按 `count`**，`3.14` 的教训）＋**每条否定型判据必带「必须为正向」的灵敏度对照**（否则「B 检索不可用」会让判据**假绿**）。依据：`3.15` 探针（[`../../probes/cross-user-probe/`](../../probes/cross-user-probe/)）**7/7 断言 PASS、退出码 `0`、两轮复跑一致**。 |
| 2026-09-24 | **`3.7`「跨用户隔离」交付**：新增 `TC-M-L1-23·1`–`·4`（**经同一门户实例双向互不可见** · **灵敏度对照（必须为正向）** · **按 id 直取不可见** · **两侧审计行各记自己的解析后库路径且互不相同**），落在**扩展既有 `make portal-mcp-session-probe`**（用户拍板 G-a ①）⇒ 断言数 **11 → 15 全 PASS、退出码 `0`**（改前基线 11/11，既有 11 条零回归）；**判据敏感性已证明**（临时把 B 的令牌换成 A 的 ⇒ 四条**全部转红**，还原后转绿）。**另修一处门禁缺陷**：四个真上游脚本的 `cleanup()` 把 `local rc=$?` 放在 `if` 之后 ⇒ **退出码恒为 `0`**、`10 / 20 / 30` 传不出来（**门禁在 CI 里永不红**）—— 已把捕获提到函数第一行，并以**合成对照**钉死（旧 ⇒ `0` · 新 ⇒ `10`）；本文件 §4-G 的「退出码」行同步如实登记。 |
