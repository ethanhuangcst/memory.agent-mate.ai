...
> **`3.6` 的自动化落点（2026-09-24，开工前置；交付时回填实测证据）**：`3.6`「全链路联通」的两条验收条件**已被既有产物覆盖大半** —— ① 「一次写入召回跑通」：`make portal-mcp-probe`（`3.1` 交付）断言「工具数 / 写入成功 / 召回命中 / 库落该用户目录」，但都在**同一会话内** ⇒ 缺「**换一个进程仍能召回**」那半条（「真的落进用户库」）；该半条已由 `3.14` 探针实测**成立**，判据登记为 `TC-M-L1-23`。② 「拒绝路径按预期失败」：**无令牌** → `TC-M-L1-02`（离线集成 + 门户侧均已验）· **越权库路径** → `3.2` 的 spawn 前断言（**fail-closed，发生在 spawn 之前** ⇒ 判据本质上在**离线层**，真上游**无观测面**，如实登记、不硬造真上游断言）。⇒ **本行的实质 = 把「跨进程召回」这条判决据挂到门户链路，并复跑收口**（代码排布见计划文件 `.codebuddy/plans/sprint4-3_6-full-chain.md`）。**另含一处顺手加固**：`portal-mcp-probe.sh` 的召回断言按 `count:1` 写（现在稳、但脆）⇒ 改为按标记判。
