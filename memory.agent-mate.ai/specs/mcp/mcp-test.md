...
| Sprint 3 #5「每用户库维护行为定档」 | TC-GC：`gc` / `curator --once` 对每用户库的覆盖面对账 | 待做 |
...
| 回归触发 | 每次改动 config（含 `[limits]`）/ 镜像 tag / 档位 / forced command 或**任一 attestation 模板**后重跑 L0 + L1 + L1.5（含 TC-ATT-01）+ L1.7（TC-LIMIT），并跑静态 `make attestation-paths`（TC-ATT-02） | 持续 |
...
| TC-GC-01 | 每库 `gc` / `curator --once` | 日志无错，且 TTL 遗忘与 WAL checkpoint 已覆盖（**覆盖面待验**） | Sprint 3 #5「每用户库维护行为定档」 |
