...
| 退出码 | `0` 全过 · `10` 前置不足 · `20` 门户起不来 · `30` 断言失败（与 [`../../scripts/portal-mcp-probe.sh`](../../scripts/portal-mcp-probe.sh) 同范式；经 `make` 时非零码会被压平为 `2`，脚本头已登记）。**2026-09-24 修一处缺陷**：此前 `cleanup()` 把 `local rc=$?` 放在 `if` 之后 ⇒ `$?` 被 `if`（条件为假 ⇒ `0`）覆盖 ⇒ **实际恒为 `0`**，上述非零码**根本传不出来**（门禁在 CI 里永不红）；现捕获在函数第一行，四个真上游脚本（`3.1` / `3.3` / `3.12` / `3.15` 的）同批修 |
