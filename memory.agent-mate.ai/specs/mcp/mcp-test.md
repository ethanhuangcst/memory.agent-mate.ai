...
> **`3.9` 的计划落点（2026-09-24，开工前置；**交付时**回填实测证据）**：`TC-M-L1-18` / `TC-M-L1-19` / `TC-M-L1-20` → `admin_portal/tests/integration/mcp-bridge.test.ts`（夹具 `tests/fixtures/fake-upstream.mjs` 需新增「可配置身份 / 能力 / 指令」与「工具内挂起」两个观测位）；机制层结论已由 `probes/bridge-identity-probe/`（`3.11`，**七项断言全过**）预先钉死，集成测试只负责「经 `/mcp` 端到端」的那一层。
