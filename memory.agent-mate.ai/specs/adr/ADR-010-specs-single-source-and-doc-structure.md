...
- 约束：`sprint-backlog.md` 与 `product-backlog.md` 由用户指定**零改动**；两份运维模板文件本次「不改、不被引用，将来移除」。
...
1. **specs 唯一真源 = `memory.agent-mate.ai/specs/`**，结构固定为：
   ...
   - 保留：`adr/`、`knowledge/`、`sprint-backlog.md`、`product-backlog.md`
...
- **为什么不改 sprint-backlog / product-backlog**：用户明确指定零改动。代价是它们内部仍指向旧文件名（74 处悬空），已在允许清单登记为已知遗留，不静默放宽规则。
...
- **已知遗留**：`sprint-backlog.md` / `product-backlog.md` 内部仍指向已合并的旧文件名，允许清单 4 条（另 2 条是待移除模板）。清理时机 = 用户决定重写这两份文件时。
