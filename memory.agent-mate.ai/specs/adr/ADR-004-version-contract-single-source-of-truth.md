...
> 编号说明：本仓 `memory.agent-mate.ai/specs/adr/` 首次建立。编号从 **004** 起，接续 `memory.agent-mate.ai/specs/vps4_new_deployment_instruction.md`
...
1. **版本契约的唯一真相源 = `memory.agent-mate.ai/upstream.lock`**（KEY=VALUE，可被 `bash`/`python3` stdlib/`make` 零依赖读取）。
...
- **为什么锁文件放 `memory.agent-mate.ai/` 而非仓根**：遵守资产隔离判据 —— 自有资产集中在 `memory.agent-mate.ai/`，
