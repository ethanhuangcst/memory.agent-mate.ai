...
Sprint 2 #4 需要一条**本地对照基线**：本机跑起 ai-memory-mcp，并**经 MCP 协议**（而非仅 CLI）完成一次写入 + 一次召回，供后续方案（隔离、门户）验证对照；Sprint 3 档位核对、Sprint 6 生产冒烟还要复用同一套判据。
...
- 脚本化而非一次性命令：基线要**可重复**；且 `initialize` 回包核对工具数这一模式正是「档位核对」与 Sprint 6 上线冒烟的判据，符合项目「探针脚本化」惯例（对照 `qwen-verify.sh` / `secret-check.sh`）。
