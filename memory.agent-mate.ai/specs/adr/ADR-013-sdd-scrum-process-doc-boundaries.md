...
- ADR-010 Decision §1 固定了当时的 specs 文档清单，但没有过程文档体例的唯一真源。当前需要新增 `sdd-scrum-practices.md`，而 ADR 按新增决议 supersede，不回写旧 ADR 正文。

## Decision

1. 新增 `sdd-scrum-practices.md`，作为 RID Registry、Sprint Backlog 与 Product Backlog 的列定义、状态语义、说明写法及单一真源边界。该文件扩展并 supersede ADR-010 Decision §1 的文档清单；ADR-010 的去重、链接和密钥纪律继续有效。
