...
| <a id="s4-mcp-session-bridge"></a>3.1 | **mcp:会话桥** | 功能 | MCP | **单次会话跑通**：持 `memo_…` 令牌经 `/mcp` 完成一次写入并召回，返回真实结果 · **令牌使用时间各自更新**：同一用户的多把令牌 `last_used_at` 独立更新（粒度 = **会话建立时一次**） · **桥的契约完整性**：`3.8` 与 `3.9` 两条增量**均已交付**（用户口径 2026-09-23：桥「能跑通」不等于「按契约可用」） | `product-backlog.md` #14 / #28 / #32 / #33 · **判据真家**：[`mcp/mcp-stories.md`](./mcp/mcp-stories.md) `MS1 AC-M1.1` / `AC-M1.2` / `AC-M1.3` · `MS3 AC-M3.1` · **用例**：[`mcp/mcp-test.md`](./mcp/mcp-test.md) §4-F `TC-M-L1-01` / `TC-M-L2-01` / `TC-M-L1-14` · [`mcp/mcp-design.md`](./mcp/mcp-design.md) §5.6.2 · [`web-portal/web-design.md`](./web-portal/web-design.md) §12.5（失败映射）· [`web-portal/web-stories.md`](./web-portal/web-stories.md) S3 `AC3.3` / `AC3.5` / `AC3.7`（门户侧视角） | 交付给：**用户**。本行是**首个对外可用**的增量。**Done（2026-09-24 恢复）**：两条**完成前置** —— `3.8`（2026-09-23）与 `3.9`（2026-09-24）—— **均已交付** ⇒ 本行的第三条验收条件（桥的契约完整性）满足，状态自 `WIP` 恢复为 `Done`。**退回经过（留档）**：2026-09-23 用户复核后本行曾自 `Done` 退回 `WIP` —— 交付后经 code review 发现两处**契约完整性缺口**，拆为 `3.8` / `3.9` 两个增量，**两者交付前本行不得置 `Done`**。缺口的实质：① 桥自称「原样转发、门户语义知识 = 0」，但 `transport.ts` 只注册了 4 个方法，其余请求由**桥自行合成** `MethodNotFound`（与直连上游的结果不一致）；② 能力与版本**硬编码**，客户端看不到真实上游（上线后无法判断镜像是否漂移）。已达成部分的落点：`admin_portal/src/bridge/` 五文件落地 —— `launch-template`（内建模板常量，逐字对照真源）· `spawn`（占位符替换 + **最小 env 白名单** + **预留 `3.2` 的 fail-closed 断言位**）· `transport`（官方 SDK 的 Streamable HTTP ⇄ stdio 直连转发，不手写帧解析）· `session`（注册表与关闭顺序）· `route`（接管 `/mcp` + 统一失败应答 + 失败写审计）。**证据指针**：离线 **297 项**全绿、覆盖率 **92.08 / 85.25 / 96.69 / 93.31**（阈值 92/85/96/93）· `make portal-mcp-probe`（真上游借壳）**四断言全 PASS、退出码 0** · 失败面 **401 / 403 / 503** 与超时兜底已断言。逐项证据与实测机制见 [`change-log.md`](./change-log.md) 2026-09-23 小节 · [`web-portal/web-test.md`](./web-portal/web-test.md) §2 的落点登记 · [`web-portal/web-design.md`](./web-portal/web-design.md) §12.5 的**超时实测三分支**。**边界（不在本行）**：`3.2` 的 fail-closed 断言只预留位置 · `3.3` / `3.4` 的验收判据 · `4.1` 的限流与超时取值 · **`500` / `429` 两类失败未实现**（属 `3.2` 与配额行 ⇒ `TC-M-L1-14` 在本行只覆盖 401 / 403 / 503 / 断开，全绿要等那两行）。 | Done |
