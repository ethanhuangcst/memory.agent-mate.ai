...
- **「验证器自己也会骗你」本轮出现三次**：断言「子进程已回收」首版用 `ps`（容器**没有 `ps`** ⇒ 恒真假绿）；改用「按 `/proc/<pid>/exe` 判二进制」又踩 **Rosetta**（所有 exe 都指向转译器 ⇒ 恒零）；最终用 **cmdline + 足量区分参数** 才成立。另有一次「库落点」断言失败，反而救出一个更严重的缺陷（见下条）。
