...
| 2 | 修 `portal-e2e.sh`：**自己启动的实例绑定失败必须 fail-loud** | 未做（现仅探测 `/healthz`，而该端点任何面都放行 ⇒ 会**静默测到别人的实例**，本次 403 即由此而来） |
...
| 6 | Issue 1–5 的收尾：方案 D（PRG + 明文暂存 60 秒）、护栏测试、覆盖率门禁复跑、原型 134 断言复跑（改了 `mockups/assets/portal.css`） | 均未做（Issue 1/2/3/5 的代码修正已完成并实测） |
