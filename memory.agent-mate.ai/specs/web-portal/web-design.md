...
> - **④ 第 ② 项的通路选择（实现前须拍板）**：门户侧目前**没有** embeddings 客户端（`admin_portal/src/**` 无 MaaS HTTP 客户端），唯一通路是 **spawn 上游子进程**；若选**直连 MaaS** 则新增配置键 ⇒ 必须同步 [`../deployment.md`](../deployment.md) 的配置契约表（`deploy-guide-audit` 的 A2 会盯住未登记的新键）。
