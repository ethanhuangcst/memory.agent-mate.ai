...
| 3 | **版本断言** | 构建期注入 `IMAGE_TAG`；启动时与挂载的 [`../../upstream.lock`](../../upstream.lock) 比对，不一致**拒绝启动**（**归属 `#2`** —— 2026-09-26 用户拍板，见下 ③） | 陈旧门户镜像在用户库被前向迁移后**静默**操作未知 schema |
...
>
> **启动自检判据定档（2026-09-25，`#4.3` 开工准备 · `3.21` 探针 8/8 PASS）**：
>
...
> - **③ 锁侧输入的归属（2026-09-26 用户拍板：整体归 `#2`；**第 2 批已落地**）**：拆成两半 —— **「构建期注入 `IMAGE_TAG`」已随 `#1` 落地**（[`../../scripts/build-portal-image.sh`](../../scripts/build-portal-image.sh) 从 `upstream.lock` 注入，探针 `D4`/`E6` 已判）；**「挂载锁文件 + 读取位」已随 `#2` 第 2 批落地**：compose 挂 `./upstream.lock:/app/upstream.lock:ro`；**读取路径写死** `/app/upstream.lock`（[`../../admin_portal/src/selfcheck.ts`](../../admin_portal/src/selfcheck.ts) 的 `LOCK_PATH`，**不进配置面** —— 它属**编排契约**，多一个键只会多一处能对不上；**测试缝** = `runSelfCheck(cfg, { lockPath })`）；**门户自身版本**由**构建期烘入**镜像（[`../../admin_portal/Dockerfile`](../../admin_portal/Dockerfile) 的 `ENV PORTAL_IMAGE_TAG=${IMAGE_TAG}`，**不在 compose 设**）。启动自检项 `own_version_matches_lock`：生产**读不到锁 = fail** · 两侧不一致 = fail · 锁被挂成**目录**（源文件缺失时 Docker 的真实表现）= **fail** · **开发姿态缺锁 = deferred**（如实登记「未判」，**不伪装通过**）。**`#17` 只做三项 `deferred` 转正**，不再认领挂载与读取位。（探针 `C5` 对挂载状态只以 `info` 记录、**不写成断言** —— 它会在落地后必然反红。）
...
| 背压 | stdio 管道与 HTTP 流**按 stream 处理**，**不整包缓冲**；对超大响应设门户级上限并**明确报错**（避免大响应击穿内存）。**`4.5` 已实现**（`4.1` 只落下了「流式不缓冲」那一半）：`PORTAL_RESPONSE_MAX_BYTES`（**未提供 = 不启用**；生产 **4 MiB**）—— 边转发边计字节，超限即**停止转发**；头**未**发出 ⇒ `502` + `RESPONSE_TOO_LARGE`，头**已**发出（SSE 中途）⇒ **截断**；两种路径**都写审计**。取值依据见 [`../../probes/response-size-probe/README.md`](../../probes/response-size-probe/README.md) |
...
| **门户自建响应体量上限**（`4.5`：单响应超过 `PORTAL_RESPONSE_MAX_BYTES`） | **视时点而定** | `RESPONSE_TOO_LARGE` | **门户自建**的背压护栏（§12.5「背压」的第②半）：头**未**发出 ⇒ `502` + 该错误码（体里附 `limit` / `sent`）；头**已**发出（SSE 中途）⇒ **截断**（客户端拿到 0 字节或半截流）。两种路径**都写审计**（`action='mcp_response_capped'`、`reason='response_too_large'`）。取值依据见 [`../../probes/response-size-probe/README.md`](../../probes/response-size-probe/README.md) |
...
| `PORTAL_RESPONSE_MAX_BYTES` | 单响应上限（背压保护）—— **`4.5` 已实现**（`4.1` 如实降级「本轮未实现」⇒ 本行补上）：边转发边计字节，超限即停止转发；头未发出 ⇒ `502` + `RESPONSE_TOO_LARGE`，头已发出（SSE 中途）⇒ 截断；两种路径都写审计。**未提供 = 不启用该护栏**；生产取值 **4 MiB**（依据 [`../../probes/response-size-probe/README.md`](../../probes/response-size-probe/README.md)：上游对 stdio 无上限、单条内容上限 `65536` 字节 ⇒ 单响应 ≈ 65 KiB；批量按 `k × 65 KiB` 估算 ⇒ 4 MiB 约 3 倍余量） | **是** |
