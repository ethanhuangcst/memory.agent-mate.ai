...
- 上游镜像来源：`ghcr.io/alphaonedev/ai-memory`（版本坐标与指纹见 `memory.agent-mate.ai/upstream.lock`）
- 首次部署 git ref：`main`
- 目标节点：野草云4（`<VPS4_IP>`）
...
- 外部依赖：仅 qwen API（**私有 MaaS 端点**，公开仓用占位符 `<QWEN_BASE_URL>`；真实值见 `memory.agent-mate.ai/secrets.local.hk_vps_4.md`）
...
**单一真相源：`memory.agent-mate.ai/upstream.lock`**（本表仅为便于阅读；任何不一致以锁文件为准）。
