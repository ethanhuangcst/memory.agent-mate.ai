...
ai-memory 的**薄部署资产**（公开父仓 `ethanhuangcst/memory.agent-mate.ai` 的 `memory.agent-mate.ai/deploy/`）：不构建镜像，只消费上游官方镜像。
...
- 方案文档（真相源）：`memory.agent-mate.ai/specs/deployment_strategy.md`（迁移后路径）
...
| `../upstream.lock` | ★ **版本坐标唯一真相源**（在 `memory.agent-mate.ai/`，不在本目录） |
...
- **端点必须显式覆盖**：`qwen` 别名默认指向公网 `dashscope.aliyuncs.com`，本项目用**私有 MaaS workspace**，故 `[llm].base_url` 与 `[embeddings].base_url` 都要写；workspace 级 key 在公网端点不通。公开仓中用占位符 `<QWEN_BASE_URL>`，真实值在 `memory.agent-mate.ai/secrets.local.hk_vps_4.md`。
...
./memory.agent-mate.ai/scripts/qwen-verify.sh

# 2) doctor（镜像仅 amd64；Apple Silicon 需显式 --platform linux/amd64）
docker run --rm --platform linux/amd64 \
  --env-file memory.agent-mate.ai/deploy/.env.local \
...
./memory.agent-mate.ai/scripts/local-up.sh

# 4) MCP 通路冒烟（docker exec -i，与生产 SSH forced command 逐字同构）
#    断言：握手 / core 档 8 工具 / 唯一标记写入 / 跨进程语义召回 + 关键词检索
./memory.agent-mate.ai/scripts/mcp-smoke.sh

# 停止本地基线（保留数据卷）：docker-compose -f memory.agent-mate.ai/deploy/docker-compose.prod.yml down
```
...
**版本坐标的唯一真相源：`memory.agent-mate.ai/upstream.lock`**（上游 tag / commit / 镜像 tag+digest / schema / 沉淀期阈值）。
