...
# 用法：bash memory.agent-mate.ai/scripts/attestation-paths-check.sh
# 退出码：0 全通过；10 模板缺失或四处口径不一致；20 发现已证伪口径残留
...
log "全部通过：四路径 attestation 口径一致，无失准表述回流"
