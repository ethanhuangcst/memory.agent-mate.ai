#!/usr/bin/env bash
# portal-mcp-session-probe.sh —— Sprint 4 `3.3`「mcp:一会话一子进程」的**真上游**端到端验收。
#
...
PORT="${PROBE_PORT:-8799}"
HANDLE_A="${PROBE_HANDLE_A:-probe33a}"
HANDLE_B="${PROBE_HANDLE_B:-probe33b}"
...
( cd "$PORTAL" \
  ...
  && npx tsx tests/fixtures/session-probe-runner.mts ) >"$TMP/runner.log" 2>&1
probe_rc=$?
...
if [ "$probe_rc" -eq 0 ]; then
  log '  OK：会话隔离判据（两会话两进程 / 无他人痕迹 / 接管不伤第三方）全部成立'
  exit 0
fi
