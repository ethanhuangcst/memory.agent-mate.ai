#!/usr/bin/env bash
# probe.sh —— Sprint 4 `3.7`「mcp:跨用户隔离」的**真上游 + 经门户**取证探针（`3.15`，**研究类，不入制品**）。
#
...
# ⇒ 本探针补的正是那一段：两个**不同用户**、两个**不同令牌**、**同一个门户实例**。
#
# 判据形状沿用 `3.14` 探针的结论：**召回类断言一律按「回包里有没有那个标记」判，禁止按 `count` 判**
# （`memory_recall` 是语义混合检索，`count` = 返回条数）。
#
...
PORT="${PROBE_PORT:-8801}"
HANDLE_A="${PROBE_HANDLE_A:-probe37a}"
HANDLE_B="${PROBE_HANDLE_B:-probe37b}"
...
cleanup() {
  ...
  # 跑完时就把临时目录与测试用户目录删掉（实测踩过）。**必须用 `if`** —— 写成 `[ ... ] && return`
  # 时条件为假会让该语句退出码为 1，下一行 `local rc=$?` 就读成 1（实测踩过）。
...
( cd "$PORTAL" \
  ...
  && npx tsx tests/fixtures/cross-user-probe.mts ) >"$TMP/runner.log" 2>&1
probe_rc=$?
...
if [ "$probe_rc" -eq 0 ]; then
  log '  OK：跨用户隔离判据（经同一门户实例的双向不可见 / 按 id 直取不可见 / 两侧审计行各记自己的库路径）全部成立'
  exit 0
fi
